...
**Memory safety (v0.17.0)**: Comprehensive memory safety hardening addressing the "pointer being freed was not allocated" crash during plugin teardown. Five fixes: (1) System malloc allocator for plugin-facing COM objects (`host_alloc.rs`) — bypasses mimalloc so plugins that incorrectly call `free()` on host objects don't trigger malloc zone mismatch aborts; (2) `AtomicBool` shutdown flag on audio engine — audio callback checks shutdown before acquiring Mutex, eliminating race window during stream teardown; (3) Sandboxed host object destruction — `HostApplication`, `HostComponentHandler` destroy calls wrapped in `sandbox_call` for defense-in-depth; (4) Defensive 50ms delay before module unload — allows plugin background threads and C++ static destructors to settle; (5) Stream-first deactivation — audio stream is stopped before engine shutdown, with 10ms drain sleep for CoreAudio in-flight callbacks. 512 tests passing.
**Dynamic analysis with Miri (v0.17.1)**: Added Miri-based dynamic analysis for detecting undefined behavior in unsafe code at runtime. Created `src/lib.rs` library crate enabling `cargo miri test --lib`, plus 21 dedicated Miri-targeted tests in `src/miri_tests.rs` exercising the highest-risk unsafe patterns: COM vtable dispatch, self-referential buffer management, struct-to-bytes reinterpretation, cross-module integration (MIDI→EventList→ProcessData), and `unsafe impl Send` thread safety. 109 tests pass under Miri with Tree Borrows, 70 under Stacked Borrows. Discovered and documented a Stacked Borrows aliasing finding in `ProcessBuffers` self-referential pointer pattern (clean under Tree Borrows). Comprehensive guide in `DYNAMIC_ANALYSIS.md`. 533 tests passing.
**AddressSanitizer (v0.17.2)**: Added AddressSanitizer (ASan) support with 46 dedicated ASan-targeted tests in `src/asan_tests.rs`. ASan instruments compiled native code to catch real hardware-level memory errors — use-after-free, double-free, heap/stack buffer overflow, memory leaks, and allocator mismatches — covering FFI code paths that Miri cannot interpret (libc::malloc/free, mmap, POSIX shm). Tests cover: host_alloc lifecycle (system malloc/free), COM object lifecycle (HostApplication, HostComponentHandler, HostPlugFrame), ProcessBuffers, shared memory, event byte reinterpretation, MIDI→ProcessData pipeline, sandbox non-crashing paths, IPC message serialization, concurrent COM objects, and full mock process sessions. 564 of 579 tests pass under ASan (15 skipped due to ASan's signal/malloc_zone interception conflicts). 579 tests passing.
...
### Test Results
- 533 unit tests passing (error Display/From, CLI parsing incl. safe-mode, types serde, scanner edge cases, cache I/O, parameter utilities, event list COM, parameter changes COM, process buffers, MIDI translation, interactive commands, host context COM, component handler concurrency, process context, COM struct layouts, IID UUID verification (incl. IPlugView/IPlugFrame), tone generator, audio device, MIDI receiver, CFBundleRef creation, IConnectionPoint IID, factory vtable layout, GUI theme colours/constants/apply, GUI app state/rack/filter/browser/backend integration/session roundtrip/device selection/parameter refresh/safe mode/transport sync/editor/param filter/audio status/param staging/selection transitions, plug_frame COM ref counting/QI/resize, editor module, sandbox crash recovery, instance-to-module crash flag communication, IPC messages serialization/wire protocol, shared memory create/open/read/write, worker state management/message handling, plugin process proxy/transport, backend process isolation/sandboxed state, Miri dynamic analysis integration tests, ASan memory safety tests)
- 579 tests total (533 standard + 46 ASan-targeted)
- Clean build with zero warnings
- Test stability verified across multiple consecutive runs
- 109 tests pass under Miri (Tree Borrows), 70 under Miri (Stacked Borrows)
- 564 tests pass under AddressSanitizer (15 skipped: signal/malloc_zone conflicts)
