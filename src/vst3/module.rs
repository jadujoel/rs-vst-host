...
//!
//! Uses types from the `vst3` crate (coupler-rs/vst3-rs) re-exported
//! via `crate::vst3::com` for COM interface definitions.
...
impl Vst3Module {
    ...
    pub fn get_info(&self) -> Result<PluginModuleInfo, Vst3Error> {
        ...
        let (vendor, url, email) = unsafe {
            if (vtbl.getFactoryInfo)(self.factory, &mut fi) == K_RESULT_OK {
                (
                    Some(char8_to_string(&fi.vendor)),
                    Some(char8_to_string(&fi.url)),
                    Some(char8_to_string(&fi.email)),
                )
            } else {
                (None, None, None)
            }
        ...
        // Enumerate classes
        let count = unsafe { (vtbl.countClasses)(self.factory) };
        ...
        for i in 0..count {
            ...
            if let Some(f2) = factory2 {
                ...
                if unsafe { (f2_vtbl.getClassInfo2)(f2, i, &mut ci2) } == K_RESULT_OK {
                    classes.push(PluginClassInfo {
                        name: char8_to_string(&ci2.name),
                        category: char8_to_string(&ci2.category),
                        subcategories: non_empty(char8_to_string(&ci2.subCategories)),
                        vendor: non_empty(char8_to_string(&ci2.vendor)),
                        version: non_empty(char8_to_string(&ci2.version)),
                        sdk_version: non_empty(char8_to_string(&ci2.sdkVersion)),
                        cid: tuid_to_bytes(&ci2.cid),
                    });
                    continue;
                }
            }
            ...
            if unsafe { (vtbl.getClassInfo)(self.factory, i, &mut ci) } == K_RESULT_OK {
                classes.push(PluginClassInfo {
                    name: char8_to_string(&ci.name),
                    category: char8_to_string(&ci.category),
                    subcategories: None,
                    vendor: vendor.clone(),
                    version: None,
                    sdk_version: None,
                    cid: tuid_to_bytes(&ci.cid),
                });
            }
        }
        ...
        if let Some(f2) = factory2 {
            let f2_vtbl = unsafe { &*(*f2).vtbl };
            unsafe {
                (f2_vtbl.base.base.release)(f2 as *mut FUnknown);
            }
        }
    ...
    fn query_factory2(&self) -> Option<*mut IPluginFactory2> {
        let vtbl = unsafe { &*(*self.factory).vtbl };
        let mut obj: *mut c_void = std::ptr::null_mut();

        let result = unsafe {
            (vtbl.base.queryInterface)(
                self.factory as *mut FUnknown,
                iid_as_tuid_ptr(&IPLUGIN_FACTORY2_IID),
                &mut obj,
            )
        };

        if result == K_RESULT_OK && !obj.is_null() {
            Some(obj as *mut IPluginFactory2)
        } else {
            None
        }
    }
    ...
    fn query_factory3_raw(
        ...
        let result = unsafe {
            (vtbl.base.queryInterface)(
                ...
                iid_as_tuid_ptr(&IPLUGIN_FACTORY3_IID),
                &mut obj,
            )
        };
        ...
        if result == K_RESULT_OK && !obj.is_null() {
            ...
            Some(obj as *mut IPluginFactory3)
        } else {
...
impl Drop for Vst3Module {
    fn drop(&mut self) {
        ...
        let result = sandbox_call("module_factory_release", move || unsafe {
            ...
            (vtbl.base.release)(factory as *mut FUnknown);
        });
...
/// Convert a null-terminated byte buffer to a Rust `String` (used in tests).
#[cfg(test)]
...
mod tests {
    ...
    #[test]
    fn test_pclass_info_w_layout() {
        // Verify PClassInfoW (vst3-rs) has the expected size.
        // cid: 16, cardinality: 4, category: 32, name: 128 (64*u16),
        // classFlags: 4, subCategories: 128, vendor: 128, version: 128, sdkVersion: 128
        // = 16 + 4 + 32 + 128 + 4 + 128 + 128 + 128 + 128 = 696
        let size = std::mem::size_of::<PClassInfoW>();
        assert_eq!(size, 696, "PClassInfoW size mismatch: got {}", size);
    }
}
